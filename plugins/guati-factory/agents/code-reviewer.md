...
name: code-reviewer
description: >-
  Use this agent when you want to review newly written code or features before
  committing them to the repository. It checks code against the project's standards
  including type safety (MyPy/Pyright strict mode), FastAPI/SQLAlchemy patterns,
  vertical slice architecture compliance, structured logging standards, and adherence
  to KISS/YAGNI principles. Trigger it after completing a logical chunk of code or a
  full feature implementation — e.g. a new service with validation and logging, a new
  API route with database operations, or a refactor whose type safety and error
  handling need checking.
model: sonnet
color: red
---
